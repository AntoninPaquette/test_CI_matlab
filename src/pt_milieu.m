function [temps , y] = pt_milieu(f , tspan , Y0 , nb_pas)
% PT_MILIEU	Methode du point milieu d'ordre 2 avec pas constant pour la
%			resolution d'EDOs
%
% Syntaxe: [temps , y] = pt_milieu(f , tspan , Y0 , nb_pas)
%
% Arguments d'entree
...
%
% Exemples d'appel
%	[temps , y] = pt_milieu('my_edo' , [0,1] , [1;0] , 1000);
%	[temps , y] = pt_milieu(@(t,y) y*cos(t) , [0,2] , 1 , 1000 );
%	[temps , y] = pt_milieu(@(t,z) [z(2);-10*z(1)] , [0,1] , [1;0] , 1000);
...
%% Methode du point milieu d'ordre 2 avec pas constant
for t=1:nb_pas
	yt			=	y(:,t);
	k1			=	h*reshape(fct(temps(t) , yt),nb_comp,1);
	y(:,t+1)	=	yt + h*reshape(fct(temps(t)+h/2 , yt + k1/2),nb_comp,1);
end
