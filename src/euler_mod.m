function [temps , y] = euler_mod(f , tspan , Y0 , nb_pas)
% EULER_MOD	Methode d'Euler modifiee d'ordre 2 avec pas constant pour la
%			resolution d'EDOs
%
% Syntaxe: [temps , y] = euler_mod(f , tspan , Y0 , nb_pas)
%
% Arguments d'entree
...
%
% Exemples d'appel
%	[temps , y] = euler_mod('my_edo' , [0,1] , [1;0] , 1000);
%	[temps , y] = euler_mod(@(t,y) y*cos(t) , [0,2] , 1 , 1000 );
%	[temps , y] = euler_mod(@(t,z) [z(2);-10*z(1)] , [0,1] , [1;0] , 1000);
...
%% Methode d'Euler modifiee d'ordre 2 avec pas constant
for t=1:nb_pas
	yt			=	y(:,t);
	y_tilde		=	yt + h*reshape(fct(temps(t) , yt),nb_comp,1);
	y(:,t+1)	=	yt + h/2*reshape(fct(temps(t),yt) + ...
										fct(temps(t)+h,y_tilde),nb_comp,1);
end
